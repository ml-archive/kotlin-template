 ...
 */
class DownloadManagerImpl(val context: Context, val client: OkHttpClient, val cacheManager: FileCacheManager, val appStateManager: AppStateManager) : DownloadManager {
    init {
    ...
    override fun downloadAttachmentContent(message : Message, content: Content) : String? {
        ...
        var folderId = if(message.folder != null) message.folder!!.id else message.folderId
        var url = "${Config.getApiUrl()}mail/folders/$folderId/messages/${message.id}/attachment/${content.id}/content".appendUserId(appStateManager.state?.impersoniateUser?.userId)
        content.contentUrlMock?.let { url = it } // if we have a mock url on the content object, use it instead
    ...
    override fun downloadReceiptContent(receiptId : String) : String? {
        ...
        var url = "${Config.getApiUrl()}channels/storebox/receipts/$receiptId/content".appendUserId(appStateManager.state?.impersoniateUser?.userId)
