 ...
 * @since    5/29/2018.
 */
class CreateDebugUserInteractorImpl(executor: Executor, val userManager: UserManager) : BaseInteractor(executor), CreateUserInteractor {
    ...
    override fun execute() {
        ...
        try {
            input?.user?.let { user->
                ...
                runOnUIThread {
                    output?.onCreateUserError(ViewError(Translation.error.genericStorageTitle, Translation.error.genericStorageMessage))
                }
            }
